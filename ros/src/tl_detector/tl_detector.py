...
class TLDetector(object):
    ...
    def waypoints_cb(self, waypoints):
        ...
        if not self.waypoint_2d:
            self.waypoint_2d = [[wp.pose.pose.position.x, wp.pose.pose.position.y] for wp in waypoints.waypoints]
    ...
    def get_closest_waypoint(self, x, y):
        ...
        Args:
            x (float64): x-coordinate of position to match a waypoint to
            y (float64): y-coordinate of position to match a waypoint to
    ...
    def process_traffic_lights(self):
        ...
        if(self.pose and self.waypoint_tree):
            ...
            end = len(self.base_waypoints.waypoints)
            for i, light in enumerate(self.lights):
                temp_wp_idx = self.get_closest_waypoint(stop_line_positions[i][0], 
                                                        stop_line_positions[i][1])
                diff = temp_wp_idx - car_wp_idx
                if diff >= 0 and diff < end:
                    end = diff
                    closest_light = light
